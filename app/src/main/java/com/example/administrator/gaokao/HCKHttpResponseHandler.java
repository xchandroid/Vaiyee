 ...
 * {@link HCKHttpClient}. The {@link #onSuccess(String)} method is designed to
...
public class HCKHttpResponseHandler {
	...
	public void onSuccess(int statusCode, Header[] headers, String content,
			String requestUrl) {
		onSuccess(statusCode, content, requestUrl);
	}
	 ...
	 *            the body of the HTTP response from the server
	 */
	public void onSuccess(int statusCode, String content, String requestUrl) {
		onSuccess(content, requestUrl);
	}
	...
	protected void sendSuccessMessage(int statusCode, Header[] headers,
			String responseBody, String url) {
		sendMessage(obtainMessage(SUCCESS_MESSAGE, new Object[] {
				new Integer(statusCode), headers, responseBody, url }));
	}
	...
	protected void handleSuccessMessage(int statusCode, Header[] headers,
			String responseBody, String url) {
		onSuccess(statusCode, headers, responseBody, url);
	}
	...
	public  void sendResponseMessage(HttpResponse response, String url) {
		...
		if (status.getStatusCode() >= 300) {
			...
			sendSuccessMessage(status.getStatusCode(),
					response.getAllHeaders(), responseBody, url);
		}
	}
}
